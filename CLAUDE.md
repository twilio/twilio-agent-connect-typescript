...
- **Voice channel initialization**: In orchestrated mode, VoiceChannel starts the conversation lookup in the background on the `setup` message (polls Conversation Orchestrator using `callSid`, extracts `profileId` from participants, then starts the local session); the first message that needs a conversation — a prompt or a DTMF keypress — awaits it. ConversationRelay creates the conversation at call-connect, so the poll overlaps the wait for the caller's first utterance instead of delaying it — which means the session and `onWebSocketConnected` can land before the caller speaks. In voice-only mode (no `conversationConfigurationId`), uses `callSid` directly as the conversation ID without polling, on that same first message.
...
- **Voice call events** (`packages/core/src/channels/voice/channel.ts`): The Calls-API callbacks — status/disposition, async AMD, recording — are a second, independently-arriving traffic class, served one route per kind under `TACConfig.voiceCallEventPath` and dispatched to the optional `onCallStatus` / `onAmd` / `onRecording` handlers. A callback URL is put on an outbound call **only when its handler is registered**, so TAC never points Twilio at a 404 (error 11200). These events carry only the `CallSid`, so `getConversationSessionByCallSid()` resolves a session by scanning `activeConversations` — and finds nothing when the event lands on an instance that never saw the call, or before the conversation lookup started at `setup` has created a session (relay-only mode: before the first prompt or keypress). `endCall()` needs no session for exactly that reason: the Twilio-side hangup still succeeds; only local session cleanup is missed.
- **Voice DTMF** (`packages/core/src/channels/voice/conversation-relay/provider.ts`): ConversationRelay sends a `dtmf` WebSocket message per keypress when `dtmfDetection` is on; `channel.on('dtmf', ...)` (or the typed `voiceChannel.onDtmf(...)`) receives them one digit at a time (accumulating multi-digit entries is the handler's job). A keypress initializes the conversation the same way a prompt does, since a caller can press a digit without ever speaking and the handler needs a session and WebSocket to respond on; if that initialization fails the digit is still delivered, with `conversationId` undefined. DTMF alone does not cancel in-flight streaming — Twilio sends a separate `interrupt` for that when `interruptible` includes `dtmf`.
...
- **Config via Zod** (`packages/core/src/lib/config.ts`): `TACConfig.fromEnv()` validates env vars. `accountSid`, `authToken`, `apiKey`, `apiSecret` are required, plus at least one of `phoneNumber` / `phoneNumbers`; `conversationConfigurationId` is optional (voice-only mode when omitted)
- **Multi-sender support**: Each channel has a singular default sender plus a plural allowlist (`phoneNumber`/`phoneNumbers`, `rcsSenderId`/`rcsSenderIds`, `whatsappNumber`/`whatsappNumbers`), back-filled from each other by a config transform (default kept first, de-duplicated). Inbound derives the agent address from the webhook's recipient (SMS/RCS/WhatsApp), validates it against the allowlist, and drops messages addressed to an unconfigured number (chat opts out via `deriveInboundAgentFromRecipients = false`). Outbound selects the sender via an optional `from` (defaulting to the channel's default sender), and digital handoff sends from the session's active agent number. Note: unlike the Python SDK, the voice provider does not yet resolve the inbound agent participant from the dialed number — only outbound `from` selection is wired for voice
