...
- **Voice channel initialization**: In orchestrated mode, VoiceChannel starts the conversation lookup in the background on the `setup` message (polls Conversation Orchestrator using `callSid`, extracts `profileId` from participants, then starts the local session); the first prompt awaits it. ConversationRelay creates the conversation at call-connect, so the poll overlaps the wait for the caller's first utterance instead of delaying it — which means the session and `onWebSocketConnected` can land before the caller speaks. In voice-only mode (no `conversationConfigurationId`), uses `callSid` directly as the conversation ID without polling, on the first prompt.
